...
export const metadata = { title: "Masa karekodları — yazdır" };
 ...
 *
 * ── Yalnızca açık masalar ───────────────────────────────────
 *
 * Kapalı masanın kodu zaten çözülmüyor; basmak, çalışmayan bir etiketi
 * masaya yapıştırmak olurdu.
 */
      ...
      <div className="mx-auto mb-8 flex max-w-4xl items-baseline justify-between gap-4 print:hidden">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold">
            {acik.length} masa karekodu
          </h1>
          <p className="mt-1 text-[13px] text-yazi-sonuk">
            Tarayıcının yazdır komutunu kullan. Kes, masaya yapıştır.
          </p>
        </div>
      ...
      {acik.length === 0 ? (
        ...
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-6 sm:grid-cols-3">
          {acik.map((m) => (
            ...
            >
              <div className="etiket-caps text-[10px] text-yazi-sonuk">{kafe?.name}</div>
